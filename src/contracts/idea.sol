...
contract IdeaTown {
    ...
    mapping(uint => mapping(address => bool)) private rated;

    /// @dev ensures caller is not the idea's creator/owner
    modifier notCreator(uint _index) {
     ...
     */
    function removeIdea(uint _index) public {
        require(
     ...
     */
    function rateIdea(uint _index, uint rate) external notCreator(_index) {
        ...
        uint totalRates = rate + currentIdea.rate;
    ...
    function supportIdea(uint _index, uint _amount)
        public
        payable
        notCreator(_index)
    {
        Idea storage currentIdea = ideas[_index];
        uint totalAmount = currentIdea.donationAmount + _amount;
